...
const UserProfilePage = () => {
  ...
  const handleConnect = async () => {
    try {
      ...
      if (currentUserRole === 'alumni') {
        ...
        setRelationship({ ...relationship, status: 'pending' });
  ...
  const renderActionButton = () => {
    ...
    if (relationship?.status === 'accepted') {
      return (
        <button 
          ...
          className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg flex items-center hover:bg-green-600 transition"
        >
    ...
    if (relationship?.status === 'none' && profile.role === 'alumni') {
      ...
      return (
        <button onClick={handleConnect} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg flex items-center hover:bg-blue-700 transition">
  ...
  if (!profile && id !== currentUserId) return <div className="mt-20 text-center">Profile not found.</div>;
  ...
  const avatarSrc = profile.profilePicture || `https://ui-avatars.com/api/?name=${profile.name}&background=0D8ABC&color=fff`;
        ...
        <div className="bg-white p-8 rounded-lg shadow-md">
          <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start mb-6 gap-4">
            <div className="flex flex-col sm:flex-row items-center sm:space-x-6 text-center sm:text-left">
              <img
                className="h-24 w-24 rounded-full object-cover border-2 border-blue-50 shadow-sm"
                src={avatarSrc}
                alt={profile.name}
              />
              <div>
